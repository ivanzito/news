...
@RequestMapping("/news")
@RestController
public class NewsControllerSSE implements HeadlinesController {

    private final Logger logger = LoggerFactory.getLogger(this.getClass().getName());

    private final SearchHeadlines searchHeadlines;

    public NewsControllerSSE(SearchHeadlines searchHeadlines) {
        this.searchHeadlines = searchHeadlines;
    }

    @Override
    //@CategoryConstraint
    @GetMapping(path = "/headlines",consumes = MediaType.TEXT_PLAIN_VALUE, produces = MediaType.TEXT_EVENT_STREAM_VALUE)
